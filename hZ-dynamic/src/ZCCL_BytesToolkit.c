...
inline void
sz_writeBits_Fast_int8(unsigned char *buffer, uint64_t *bitPosPtr, int numBits, unsigned char data)
{
    unsigned char mask = (1 << numBits) - 1;
    *(buffer + ((*bitPosPtr) >> 3)) |= (data & mask)
        << ((*bitPosPtr) & (uint64_t) 0x0000000000000007);
    (*bitPosPtr) += numBits;
}

inline void
sz_writeBits_Fast_int32(unsigned char *buffer, uint64_t *bitPosPtr, int numBits, int32_t data)
{
    uint32_t mask = (1 << numBits) - 1;
    *(uint32_t *) (buffer + ((*bitPosPtr) >> 3)) |= ((*(uint32_t *) &data) & mask)
        << ((*bitPosPtr) & (uint64_t) 0x0000000000000007);
    (*bitPosPtr) += numBits;
}

inline void
sz_writeBits_Fast_int64(unsigned char *buffer, uint64_t *bitPosPtr, int numBits, int64_t data)
{
    uint64_t mask = ((uint64_t) 0x0000000000000001 << numBits) - 1;
    *(uint64_t *) (buffer + ((*bitPosPtr) >> 3)) |= ((*(uint64_t *) &data) & mask)
        << ((*bitPosPtr) & (uint64_t) 0x0000000000000007);
    (*bitPosPtr) += numBits;
}
...
inline unsigned char numberOfLeadingZeros_Long(long i)
{
    ...
    if (((unsigned int) x) >> 24 == 0) {
        n += 8;
        x <<= 8;
    }
    if (((unsigned int) x) >> 28 == 0) {
        n += 4;
        x <<= 4;
    }
    if (((unsigned int) x) >> 30 == 0) {
        n += 2;
        x <<= 2;
    }
    n -= ((unsigned int) x) >> 31;
