                                      ...
                                      float relBoundRatio, size_t r5, size_t r4, size_t r3, size_t r2, size_t r1)
{
    ...
    if (dataType == SZ_FLOAT)
    {
        ...
        if (errBoundMode == REL)
        {
            ...
            if (fastMode == 1)
            {
                bytes = ZCCL_float_openmp_threadblock(oriData, outSize, realPrecision,
                                                     length, blockSize);
            }
            else if (fastMode == 2)
            {
                bytes = ZCCL_float_openmp_threadblock_randomaccess(oriData, outSize, realPrecision,
                                                                  length, blockSize);
            }
        }
        if (fastMode == 1)
        {
            bytes = ZCCL_float_openmp_threadblock((float *)data, outSize, realPrecision,
                                                 length, blockSize);
        }
        else if (fastMode == 2)
        {
            bytes = ZCCL_float_openmp_threadblock_randomaccess((float *)data, outSize, realPrecision,
                                                              length, blockSize);
        }
    }
    else
