                                       ...
                                       MPI_Comm comm)
{
    ...
    if (rank == root && root == 0)
    {
        ...
        {
                                   ZCCL_float_single_thread_arg((char *)outputBytes + offset, (char *)sendbuf + i * sizeof(recvtype), &outSize, absErrBound, sendcount, blockSize);
    ...
    else if (rank == root && root != 0)
    {
        ...
        for (i = 0; i < curr_cnt; i += sendcount)
        {
                                   ...
                                   ZCCL_float_single_thread_arg((char *)outputBytes + offset, (char *)tmp_buf + i * sizeof(recvtype), &outSize, absErrBound, sendcount, blockSize);
            compressed_sizes[j] = outSize;
            compressed_offsets[j] = offset;
            offset += outSize;
            j++;
    ...
    if ((relative_rank % 2))
    {
                                 ZCCL_float_decompress_single_thread_arg(recvbuf, recvcount, absErrBound, blockSize, (char *)outputBytes);
    }
                                       ...
                                       MPI_Comm comm)
{
    ...
    if (rank == root && root == 0)
    {
        ...
        for (i = 0; i < curr_cnt; i += sendcount)
        {
                                   ZCCL_float_openmp_threadblock_arg((char *)outputBytes + offset, (char *)sendbuf + i * sizeof(recvtype), &outSize, absErrBound, sendcount, blockSize);
            compressed_sizes[j] = outSize;
            compressed_offsets[j] = offset;
            offset += outSize;
            j++;
    ...
    else if (rank == root && root != 0)
    {
        ...
        for (i = 0; i < curr_cnt; i += sendcount)
        {
                                   ...
                                   ZCCL_float_openmp_threadblock_arg((char *)outputBytes + offset, (char *)tmp_buf + i * sizeof(recvtype), &outSize, absErrBound, sendcount, blockSize);
            compressed_sizes[j] = outSize;
            compressed_offsets[j] = offset;
            offset += outSize;
            j++;
    ...
    if ((relative_rank % 2))
    {
                                 ZCCL_float_decompress_openmp_threadblock_arg(recvbuf, recvcount, absErrBound, blockSize, (char *)outputBytes);
    }
    if ((rank == root) && (root == 0) && (recvbuf != MPI_IN_PLACE))
    {
        memcpy(recvbuf, sendbuf, sendcount * sizeof(sendtype));
